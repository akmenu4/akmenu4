...
void CIniFile::SetStringVector(const std::string& Section, const std::string& Item,
                               std::vector<std::string>& strings, char delimiter) {
    std::string strValue;
...
bool CIniFile::SaveIniFile(const std::string& FileName) {
    ...
    for (size_t ii = 0; ii < m_FileContainer.size(); ii++) {
        ...
        if (notSpace != std::string::npos) strline = strline.substr(notSpace);

        if (strline.find('[') == 0 && ii > 0) {
            if (!m_FileContainer[ii - 1].empty()) fprintf(f, "\n");
        }
                ...
                if (strSection == Section) {
                    while (ii < iFileLines) {
                        ...
                        if (equalsignPos != strline.npos) {
                            size_t last =
                                    equalsignPos ? strline.find_last_not_of(" \t", equalsignPos - 1)
                                                 : strline.npos;
...
void CIniFile::SetFileString(const std::string& Section, const std::string& Item,
                             const std::string& Value) {
    std::string strline;
    std::string strSection;
    std::string strItem;
        ...
        if (rBracketPos > 0 && rBracketPos != std::string::npos) {
            ...
            if (strSection == Section) {
                while (ii < iFileLines) {
                    ...
                    size_t equalsignPos = strline.find('=');
                    if (equalsignPos != strline.npos) {
                        size_t last = equalsignPos
                                              ? strline.find_last_not_of(" \t", equalsignPos - 1)
                                              : strline.npos;
                        if (last == strline.npos)
                            strItem = "";
                        else
