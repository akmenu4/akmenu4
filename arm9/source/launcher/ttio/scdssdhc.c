...
static u32 SCDS_SDHostSetMode(u8 sdio, u32 parameter, u8 response_type, u32 latency) {
    u64 command = ((u64)SCDS_CMD_SD_HOST_PARAM << 56) | ((u64)parameter << 24) | ((u64)sdio << 16) |
                  ((u64)response_type << 8);
...
bool SCDS_SDInitialize(void) {
    ...
    // Init
    SCDS_SDHostSetRegister(SCDS_SD_HOST_REG_RESET | SCDS_SD_HOST_REG_400KHZ_CLK |
                           SCDS_SD_HOST_REG_CLEAN_ROM_MODE);
    ...
    if (SCDS_isSDHC)
        SCDS_SDHostSetRegister(SCDS_SD_HOST_REG_RESET | SCDS_SD_HOST_REG_CLEAN_ROM_MODE |
                               SCDS_SD_HOST_REG_SDHC);

    return true;
}

void SCDS_SDReadSingleSector(u32 sector, void* buffer) {
    u64 command = ((u64)SCDS_CMD_SD_READ_SINGLE_BLOCK << 56) |
                  ((u64)(SCDS_isSDHC ? sector : sector << 9) << 24);
    card_romSetCmd(command);
    card_romStartXfer(SCDS_CTRL_READ_4B, false);
    card_romWaitDataReady();
    card_romGetData();
...
void SCDS_SDReadMultiSector(u32 sector, void* buffer, u32 num_sectors) {
    u64 command = ((u64)SCDS_CMD_SD_READ_MULTI_BLOCK << 56) |
                  ((u64)(SCDS_isSDHC ? sector : sector << 9) << 24);
    card_romSetCmd(command);
    card_romStartXfer(SCDS_CTRL_READ_4B, false);
    card_romWaitDataReady();
